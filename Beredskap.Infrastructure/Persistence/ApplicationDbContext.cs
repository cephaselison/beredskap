...
using Beredskap.Common.HelperClasses;
...
namespace Beredskap.Infrastructure.Persistence
{
    ...
    public class ApplicationDbContext : IdentityDbContext<ApplicationUser>
    {
        ...
        public ApplicationDbContext(ITenantService tenantService,DbContextOptions<ApplicationDbContext> options, IHttpContextAccessor httpContextAccessor) : base(options)
        {
            ...
            var encryptionKey = "vkQXRcH1tXmMtyQiKnU+lkLdQw8ARQUh5qOgHrbNyHE=";
            var ivKey = "yIaXToqvSZJg65vZxO/0hA==";
            var t = AesProvider.GenerateKey(AesKeySize.AES256Bits);
            _provider = new AesProvider(Convert.FromBase64String(encryptionKey), Convert.FromBase64String(ivKey));
        }
        protected override void OnModelCreating(ModelBuilder builder)
        {
            ...
            builder.Entity<VenueEntity>().HasQueryFilter(a => a.TenantId == CurrentTenant);
            builder.Entity<Incident>().HasQueryFilter(a => a.TenantId == CurrentTenant); 
        ...
        public string Encrypt(object data)
        {
            try
            {
                var target = data?.ToString() ?? string.Empty;
                if (string.IsNullOrEmpty(target)) return target;

                var result = Encoding.ASCII.GetBytes(target);
                var byteArray = _provider.Encrypt(result);
